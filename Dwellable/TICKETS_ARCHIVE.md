# Dwellable Native — Ticket Log Archive

**Historical session-log entries from `TICKETS.md`, moved here August 24, 2026 to cut session-start read cost.** These are dated prose entries only — the live ticket registry (per-ticket status/priority/epic) is `TICKETS.csv`; the current backlog is in `TICKETS.md`. Read this file only when you need detail on a specific past session; otherwise `TICKETS.md`'s recent entries + `docs/MEMORY.md` are enough context to resume work.
