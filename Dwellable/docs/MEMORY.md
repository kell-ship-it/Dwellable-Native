...
**Pillar 0 is done — no open items.** Next design-phase work is whichever pillar Kell wants to pick up next (Pillar 1 Capture screens would be a natural continuation, given `capture-method`'s content is already half-built and just needs the right home). No blocking items carried forward from this session.
