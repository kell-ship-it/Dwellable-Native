...
**Pillar:** Authentication | **Updated:** July 22, 2026 (password recovery reframed — see note below) | **Status:** Strategy Locked

**🔄 Model note (July 22, 2026):** Pillar 2 moved from client-side E2E encryption (password-derived key) to server-side encryption (server-managed key, password used for authentication only). This means the password no longer has any role in encryption key derivation — every reference below to "EncryptionManager derives key from password (Argon2id)" is removed. The practical effect: **password reset/change no longer risks data loss**, and account lockout / rate limiting is now safe to implement without risking permanently orphaning a user's data. See `docs/PILLAR_2_SECURITY_STRATEGY.md` for full rationale.
...
|----------|--------|-----------|
| **Auth provider** | Supabase (JWT-based) | Simple, serverless; password is authentication-only, independent of Pillar 2's server-managed encryption key |
| **Session management** | JWT tokens (client-side) | Stateless; scales without backend session store |
| **Password strength** | 8+ chars, mixed case, number, symbol | Standard account-security baseline (no longer tied to encryption key derivation) |
| **Account lockout (MVP)** | Basic rate limiting (extends T-050) | Now safe to implement — password reset/lockout no longer risks orphaning a user's encryption key; full Phase 2 hardening still deferred |
...
- User logs in with new password → full access restored immediately

**P2 Implication (changed July 22, 2026):**
- Because the encryption key is server-managed (not password-derived), a password reset has **zero impact on data access**. Every moment — old and new — is immediately readable after reset. No asymmetry, no re-encryption, no data loss.
...
- Validation: verified against Supabase auth
...
- Updates Supabase auth table (new password hash)
- No key re-derivation needed — encryption key is unaffected by password change
...
**Encryption Key Handling (P2 Implication, updated July 22, 2026):**
- During soft-delete: the server-managed encryption scope for this user is marked inactive (not destroyed)
- If account restored: reactivated automatically (user can access moments again)
- If 30 days expire: the user's encrypted data and its access path are permanently destroyed (irreversible) — this is now the actual point of no return, not password/key loss
...
| **Account lockout after N attempts** | Standard security pattern | No longer conflicts with the encryption model (password reset doesn't touch data access); still deferred to Phase 2 for scheduling, not architectural reasons |
...
| **Passwordless auth (magic links)** | Reduces password burden | Adds backend complexity (token generation, email delivery); standard password auth simpler for MVP |
...
| **User forgets password after signup** | Locked out until reset | Not a data-loss risk under the server-side model — password reset via email restores full access with zero impact on stored data |
| **Brute-force attacks on login** | Account compromised | Basic rate limiting now safe to implement (no longer risks orphaning a user's data); full Phase 2 hardening (suspicious-activity alerts) still deferred |
...
| **Multi-device sync** | Not yet designed | Simpler under server-managed keys than it would have been under E2E; still a Phase 2+ design task |
...
| **Encrypted data not destroyed on hard-delete** | Data potentially recoverable | Ensure server-side encrypted data and its access path are actually purged on hard-delete; verify in audit trail |
...
- **Pillar 2 (Security & Privacy):** Encryption key is server-managed, independent of the user's password — password reset/change has no impact on data access
...
| **Account Lockout** | Basic rate limiting MVP-safe (extends T-050); full hardening Phase 2 |
| **Account Recovery (Password)** | Email-based password reset with 30-min expiry tokens — now zero data-loss risk (see P2 model change) |
| **Account Deletion** | Soft-delete with 30-day recovery window; hard-delete after 30 days |
| **Encryption Integration** | Password authenticates only; encryption key is server-managed (P2), independent of password — reset/change has no data impact |
