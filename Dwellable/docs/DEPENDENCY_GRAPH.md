...
|------|-----------|-------------|
| **LLM selection not locked** | T-092: Lock Gemini 2.0 Flash (MVP) → Mistral 7B (scale) by end of week 1 | CTO/LLM owner |
| **Supabase auth setup delayed** | Have Supabase project + auth tables ready before Auth pillar starts | Infra engineer |
| **Encryption T-062 not complete** | P2 must complete before P1 ships data (by end of week 4) | Crypto engineer |
| **LLM API integration slow** | P3 prayer generation needs low-latency LLM calls (<3s); test early | Backend/LLM owner |
...
| **Critical Blocker** | LLM selection + Supabase setup (needed by week 1) |
