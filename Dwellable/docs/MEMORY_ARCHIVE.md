# Dwellable Native — Session Memory Archive

**Historical session entries from `docs/MEMORY.md`, moved here August 24, 2026 to cut session-start read cost.** Read this file only when you need detail on a specific past session; otherwise `docs/MEMORY.md`'s recent entries are enough context to resume work.
