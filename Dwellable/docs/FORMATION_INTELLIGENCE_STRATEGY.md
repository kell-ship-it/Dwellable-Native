...
| **P2 (Security)** | Trust Layer | Protect moments with server-side encryption at rest so rich context can be used safely, with transient decrypt for processing. Protection IS trust. |
...
- Privacy guarantee aligns with Dwellable's data-stewardship philosophy (server-side encryption at rest, transient decrypt for processing — see docs/PILLAR_2_SECURITY_STRATEGY.md)
...
3. ✅ **P2 (Security):** Server-side encryption at rest (locked, ready to build)
