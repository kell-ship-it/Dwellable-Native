...
**Note:** the `object` field above is stale per T-154 (dropped) — flagging inline here, but the full struct rewrite is out of scope for this doc's current pass.

### Formation Intelligence Integration (Row 3 & Row 5)

- **Row 3 (chained Call B):** after this pillar's Journal Synthesis (Call A, above) completes, Formation Intelligence's own Theme Detection (Call B) runs afterward as a separate, chained call — P4 does not implement Call B itself, it only produces the finished journal Call B reads.
- **Row 5 (journal edits are a no-op for MVP):** edits made here post-creation (headline, tags, mood, body) do **not** feed Formation Intelligence in MVP. This is an explicit scope decision confirmed this session, not a permanent rule — open to revisiting Post-MVP.

Full mechanism: see `docs/PILLAR_6_FORMATION_INTELLIGENCE_TECHNICAL_SPEC.md`, Row 3 and Row 5.
