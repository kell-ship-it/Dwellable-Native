# Pillar 3: Prayer — Strategy & Design Skeleton
...
| **P3 (Prayer)** | **Seals the moment spiritually** (affirms understanding, deepens engagement) |
...
## Re-Dwelling & Multi-Session Prayer (MVP Clarification)

**Critical Clarification for Entry Points (Today, Entries, Growth, Notifications):**

P3 owns **not just post-capture prayer, but all prayer/prayer experiences** — regardless of entry point or timing.

### When Re-Dwelling Happens

Users initiate prayer/prayer in multiple contexts:
...
| **Daily invitation** | Today tab (unprayed moment) | User sees unprayed moment → taps → Prayer flow |
| **Theme exploration** | Growth tab (tap a theme) | User sees theme → taps individual moment within theme → Prayer flow |
| **Search/discovery** | Entries tab (tap moment) | User finds moment via search → taps moment detail → Prayer flow available |
| **Notification trigger** (Post-MVP) | Push notification | User taps theme breakthrough notification → routed to moments → Prayer flow |

### Re-Dwelling UX (MVP Scope)

**Question: Is re-dwelling UX identical to post-capture prayer, or different?**

**Answer (Locked):** Identical UX for MVP. Both use the same **Prayer Flow:**
...
**Key difference:** Context passed into Prayer Flow varies by entry point:
...
**Output is same:** "Prayed" signal added to moment, prayer_count increments, depth signal added to Formation Intelligence.
...
- **MVP:** Users can only "Pray" once per moment (first engagement creates prayer signal)
- **Post-MVP:** Enable re-prayer on same moment
  - Track prayer_count (how many times user has prayed about this moment)
  - Show "Dwell History" (all times user engaged with this moment)
  - Each new prayer adds to formation intelligence (what changed? what deepened?)
