# Pillar 7: Beta & Marketing — Strategy & Design Specification

**Founder:** Kell Golden | **Status:** ✅ Design Locked | **Updated:** July 23, 2026 (all 5 tentative decisions locked; tooling synced to match ARCHITECTURE.md)

**Naming note:** This file is still named `P8_BETA_MARKETING_STRATEGY.md` from before the May 10, 2026 pillar renumbering (old "Pillar 8" → current **Pillar 7**). Content below reflects current numbering; filename left as-is for now to avoid breaking existing links — flagged for a future cleanup pass alongside the other stale-numbered files in `docs/`.
...
1. ✅ **Waitlist Management:** FIFO (first come, first served) — simpler to implement and explain to users than practice-type segmentation
2. ✅ **Beta Duration:** 8 weeks — matches the WAR success metric's own evaluation point ("40-50% by week 8"), so the beta's length and its success gate line up naturally
3. ✅ **Control Group:** No control group — too complex for beta scale (20-30 users/cohort); measure engagement variance across cohorts instead of a true A/B holdout
4. ✅ **Referral Bonus:** Thank-you only, no incentive — keeps the beta's tone consistent with Dwellable's non-transactional, formation-first brand; avoids gamifying a spiritual practice
5. ✅ **Beta Compensation:** **Early access to a lifetime deal** — active beta participants get first/priority access to purchase a lifetime-access tier (paid, at a locked-in price) before it opens to the general public. Not a free giveaway — the reward is priority + pricing lock-in, not comped access.
...
**Corrected July 20/23, 2026:** The original PostHog/Segment/Metabase/Grafana recommendations below contradicted the locked `ARCHITECTURE.md` decision against third-party analytics SDKs (user privacy — see Analytics row in ARCHITECTURE.md's Key Decisions table). Corrected to extend Dwellable's existing in-app tooling instead.

1. **TestFlight Distribution:** Apple TestFlight for app delivery
2. **Analytics Platform:** Extend existing in-app `UsageTracker` + Supabase (not PostHog/Segment) — same extension already needed for Pillar 8 (Notifications) and Pillar 11 (Growth tab) event tracking
3. **Survey Platform:** Typeform, Qualtrics, or in-app surveys
4. **Community Platform:** Discord server for beta community
5. **Database:** Supabase (existing backend) with cohort tracking tables
6. **Email Platform:** Mailchimp, Braze, or custom email service
7. **Metrics Dashboard:** Extend the existing analytics dashboard pattern (from T-059) rather than adopting Metabase/Grafana
...
- ✅ All 5 tentative decisions locked (July 23, 2026)
- ✅ Tooling corrected to match ARCHITECTURE.md (no third-party analytics SDKs)
...
**Pillar design sequence: P9 ✅ → P10 ✅ → P11 ✅ → P8 ✅ → P6 ✅ → P2 ✅ → P7 ✅ (locked July 23, 2026) — all pillar design work complete. Next: implementation ticket sequencing, starting with T-062 (Server-Side Encryption).**
