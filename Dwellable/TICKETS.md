...
- [ ] **T-125:** Crisis Protocol, Chatbot-Misuse Guardrails & Resource-Link Strategy 🔲 **NOT STARTED**
  - **🚨 RECLASSIFIED TO MVP (July 23, 2026):** Previously deferred to "Pillar 6," which was a mismatch — P6 is Formation Intelligence (pattern detection), not a moderation/safety pillar, and this was effectively an orphaned deferral with no real owner or target. Given the legal/safety exposure of an app inviting free-form emotional disclosure without a launch-ready crisis response, **this moves to MVP scope** — a basic crisis-response guardrail (detect self-harm/crisis language, surface a resource/hotline, respond with care rather than continuing the conversation as normal) ships with P3 at launch. This is a safety floor, not a feature to defer. The fuller Formation Intelligence tie-in (tracking `crisis_signal` for wellbeing-pattern surfacing, deliverable 7 below) can still phase in with P6, but the core detect-and-respond guardrail cannot wait for P6.
  ...
  - **Dependencies:** P3 (prayer generation must not halt on crisis content); P1 (capture must not refuse to save such moments); P2 (encryption must protect this sensitive data). Interlocks with token caps T-108/T-119. Crisis-pattern tracking (deliverable 7) is a Post-MVP consumer of P6 once it exists — not a blocker for the MVP guardrail itself.
