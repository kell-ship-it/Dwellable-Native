...
- [ ] **T-076:** Build SwiftUI NavigationStack with 4 tabs 🔲 **NOT STARTED (design pass complete)**
  ...
  - **Design pass complete (August 20, 2026, later same-day session):** New Figma page `P6 — Menu / Navigation` (file `t5MUGEtpeFcUixobvHiYMc`) created — see T-174 for the "Pillar 6" naming decision this page name reflects. Composed into 4 full 402×874 screens (Today/Entries/Capture/Growth), each with the tab bar pinned to the bottom and the correct tab highlighted gold. The Capture screen reuses the actual `Logo-Group` asset cloned from the P1 Error States page for visual fidelity, recreating the real Capture-home screen (flame logo, "What's stirring in you today?", suggestion pills, input bar) now shown reachable from inside the tab shell — this is the concrete "Menu makes Capture reachable" proof the Aug 20 session's rationale called for. Today/Entries/Growth screens are explicitly labeled placeholder content owned by their respective pillars (P10/P5/P11) — only the shell itself (tab bar + navigation) is this ticket's actual scope. **Still not started:** no SwiftUI code exists yet: this is Figma design only, same "design validated, not yet ported" pattern as T-172/T-173.
  - **v2 redesign (same session, per Kell's reference image):** Replaced the initial full-width bottom bar with a floating rounded-pill `TabBar` (362×96, 34px corner radius, drop shadow, inset 20px from screen edges and 24px from the bottom) — matches a reference screenshot Kell provided directly. Rebuilt all 4 icons: Today (ring + center dot), Entries (open-book glyph, built from two flat-topped rounded rectangles rather than SVG arc paths — Figma's `vectorPaths` parser rejected arc (`a`) commands, rect-based approach was the reliable fallback), **Capture** (outlined circle + plus, gold ring when active — not the earlier filled-gold-circle treatment), Growth (3 ascending bars, unchanged). **3rd tab relabeled "Create" → "Capture"** per Kell's reference — screen frame and all copy renamed to match. Old v1 TabBar components/instances deleted, not left as dead nodes.
  - **v3 structural change (same session, per Kell reviewing a Journey app screen recording):** Capture pulled out of the tab bar entirely. Journey (a real journaling app Kell recorded and shared) doesn't put "create entry" in its tab bar at all — it's a separate "+" button in the header, opening a fully independent full-screen compose view with zero tab-bar presence. Rebuilt to match: tab bar is now **3 tabs (Today | Entries | Growth)**, plus a persistent gold `HeaderPlusButton` component (top-right, present on all 3 tabs) that launches Capture as its own full-screen flow. This also resolves the "tab bar and an active text field visible together" complaint Kell raised — Capture is now never a tab's content, so the collision can't happen by construction. New components: `TabBar v3 (3-tab)` (3 active-state variants) and `HeaderPlusButton — Launch Capture`, both on the `P6 — Menu / Navigation` page. The Capture entry screen (real `B-guided-prompt-home` design, cloned from `P1 — Capture Onboarding`) got a dismiss "×" top-left since it's now a true full-screen modal, not a tab.
  - **Exit-friction pattern added (same session):** Kell flagged that an easy, un-confirmed exit from an in-progress reflection "feels weird" — too easy to abandon a moment mid-capture. Built a gentle exit-confirmation bottom sheet: the dismiss "×" exits instantly when the entry screen is empty (nothing to lose), but once there's actual content (typed text or an active recording), tapping "×" surfaces a sheet — "Leave before finishing?" / "What you've shared so far will be lost if you leave now." — with a big gold "Keep Reflecting" primary button and a small, quiet "Discard and exit" text link below it. Friction by visual asymmetry (safe path big and obvious, destructive path small and deliberate), not by blocking the exit outright. Applied to both the recording and typing states on `P1 — Capture — Post-Onboarding`.
  - **Transition mechanism — LOCKED: Container Transform (August 20, 2026).** Kell asked "how does the + button actually get you into Capture" — researched via Mobbin/Material Design (`Full-screen Overlay` and `Floating Action Button` glossary entries) and found this needs a real transition, not a flat cut. Built and published an interactive HTML prototype (`capture_transition_prototype.html`, Artifact) with 7 real animated options to compare live rather than static frames: Container Transform, Iris Reveal, Corner Curtain, Cross-fade + Scale, Slide Up, Slide Down, Slide Left→Right. Fixed a real bug during that build — the capture layer had no background of its own for 4 of the 7 variants (only child cards did), so the idle screen bled through and looked jumbled; fixed by giving every variant a shared opaque base layer. Claude's recommendation was **Slide Up** (matches Journey's own actual behavior, free via SwiftUI's native `.fullScreenCover`, far less engineering risk than shared-geometry animation) but **Kell explicitly chose Container Transform instead** — the "+" button itself expands and unfolds into the full screen (Material Design's own documented pattern for compose actions, e.g. Gmail/Google Keep). Will need `matchedGeometryEffect` or iOS 18's `.zoom` navigation transition at implementation time. Figma's 4-option comparison section (`P1 — Capture — Post-Onboarding` page) relabeled to mark Container Transform as chosen, other 3 kept as "considered, not chosen" for the record.
  - **Real screens ported in + "Start over?" modal (same session).** Replaced the fabricated conversation/save screens with the actual approved designs from `P1 — Capture Onboarding — Full Flow (Polished v1)` (Recording, Live Transcription, Conversational Review, Conversational Loop, Entry Saved, Creating Your Journal, Journal Reveal) — a repeated correction from Kell this session after catching invented content twice. Added a "Start over?" modal for the mid-conversation exit case specifically (distinct from the plain X-exits-freely case before any message is sent) — center-screen card per Kell's preference (not a bottom sheet), copy "If you leave now, this conversation will be lost and you'll need to start over," gold **Save Moment** as primary (Kell's own edit, replacing an earlier "Keep Talking" label — commits early and routes to the Saved screen rather than just resuming the chat) and a quiet **Start Over** text link that actually discards. Confirmed with Kell: no draft/autosave system exists anywhere in the app — leaving without an explicit save action genuinely loses the content, so the warning is accurate, not just punitive framing. Error states (T-172's 8 scenarios) are NOT rebuilt for post-onboarding — identical to onboarding's, with the one difference that the exit modal applies here (onboarding has no in-app way to leave at all).
  - **🚨 Prototype format correction (same session):** Kell corrected that "prototype" in this project always means an HTML file, never a Figma Present-mode click-through — a wired Figma prototype was built first and was the wrong deliverable. Rebuilt as `docs/P1_PostOnboarding_Prototype.html`: 3 blank tab screens (Today/Entries/Growth) each with the persistent "+" button, wired via a `data-goto`/`data-overlay` nav engine into the full real-content loop (Entry → Recording → Transcription → Conversational Review → Conversational Loop → Start Over modal → Saved → Creating Journal → Journal Reveal). Click-tested live in the Browser pane end-to-end (not just visually inspected) — caught and fixed one real bug in the process: the modal's headline was invisible because `.modal-card` never set its own text color and was inheriting the hub panel's dark text instead of the phone screen's white. Committed to `docs/`, not left in scratchpad — see new `docs/PROTOTYPES.md` registry (created this session after finding T-172's onboarding prototype was described in tickets but never actually committed anywhere) and its standing rule to always build HTML and verify it by actually clicking through before calling it done.
...
### Pillar 0 — Onboarding Prototype Follow-ups (August 4, 2026 session)

- [ ] **T-170:** Review Onboarding Reveal Pacing Against Pillar 1 Capture Flow — Pillar 0 🔲 **NOT STARTED**
  - **Purpose:** The reader-paced timed-reveal pacing across `docs/P0_NavChrome_Prototype.html` (s05-s20) was locked screen-by-screen in isolation. Needs a pass confirming it still feels right once experienced back-to-back with real Pillar 1 (Capture) UX, once that exists.
  - **Dependencies:** Pillar 1 (Capture) design/build — **UPDATED August 20, 2026: no longer "nothing to test against" — the full happy-path + all 8 error-state scenarios now exist as a working, click-through HTML prototype (see T-172). Not a native build yet, but enough exists to actually run this comparison now.**
  - **Estimated effort:** S–M
  - **Priority:** 🟡 MEDIUM
  - **Raised:** August 4, 2026 session (Reader-Paced Onboarding Animation build)

- [ ] **T-171:** Review Loading-Screen Content for Pillar 0 / s20 — Pillar 0 🔲 **NOT STARTED**
  - **Purpose:** The 4 step labels on the `moment-types-loading` (s20) auto-advancing sequence ("Setting up your journal...", etc.) currently imply SDK downloads/setup that don't actually happen at runtime — WhisperKit is bundled at install (T-097) and Supabase/Firebase SDKs are compiled into the binary, not fetched at this point.
  - **Deliverables:** Copy pass to either better dramatize what's actually real (account creation) or confirm the copy is intentionally poetic rather than literal.
  - **Dependencies:** None — low effort, can be picked up opportunistically
  - **Estimated effort:** S
  - **Priority:** 🟡 MEDIUM
  - **Raised:** August 4, 2026 session (Reader-Paced Onboarding Animation build)
...
### Pillar 1 — Capture Error States & Navigation Sequencing (August 20, 2026 session)

- [x] **T-172:** Pillar 1 (Capture) Error States & Edge Cases — HTML Prototype (Scenarios 1-8) ✅ **COMPLETE (prototype only — see note)**
  - **What was built:** All 8 error/edge-case scenarios from the Figma error-states file, integrated into the same working `capture_flow_prototype.html` used for the happy-path build (not a separate demo file) — mic permission denied, transcription silence (1st + escalated 2nd attempt), technical transcription failure (retry-same-audio, then same escalation as silence), max recording duration (silent auto-stop, full-screen long-transcript review, mic-as-resume append), Dwelly offline mid-reply (reconnects vs. stays-offline-finish-via-Done, persistent status banner), and Cancel-vs-Stop while recording (real bug fix — Cancel previously had no handler and silently acted like Stop).
  - **⚠️ Important scope note:** This is an **HTML/JS prototype**, not Swift. `CaptureView.swift` (S-003, already ✅ Complete per the March build) does not yet implement any of these 8 error paths. Treat this ticket as "design + interaction spec validated," not "shipped."
  - **Dependencies:** None
  - **Estimated effort:** N/A (prototype, already done)
  - **Priority:** 🟡 MEDIUM (until scheduled for native build)
  - **Raised/Completed:** August 20, 2026 session

- [ ] **T-173:** Port Validated Capture Error States into Native `CaptureView.swift` 🔲 **NOT STARTED**
  - **Purpose:** T-172's 8 error-state scenarios are validated in prototype form (interaction pattern, copy, timing) but not implemented in the actual iOS app. `CaptureView.swift` currently has no handling for mic-denied, transcription failure, max-duration, or Dwelly-offline states.
  - **Dependencies:** T-172 (done — use the prototype as the interaction spec), shared LLM/transcription infra (T-168 and related)
  - **Estimated effort:** L
  - **Priority:** 🟡 MEDIUM
  - **Raised:** August 20, 2026 session

- [x] **T-174:** Resolve "Pillar 6" Naming Collision for Menu/Navigation ✅ **RESOLVED**
  - **Purpose:** Kell asked "what pillar is the menu?" this session. The honest answer has drift: `PILLAR_6_MENU_BAR_STRATEGY.md` and the "Pillar 6 (Menu Bar) Implementation Tickets (May 6 Session)" section (T-076–T-081, this file) both label Menu/Navigation as **Pillar 6** — but that's the *original* May 6 ticket-numbering scheme. A later, more authoritative pass (per `docs/MEMORY.md`'s July 21, 2026 entry and its own Source-of-Truth Index) locked a **different "Pillar 6" = Formation Intelligence**, and that same Index files Menu/Navigation under **"Supporting Pillars (added May 14, not in core numbering)"** — i.e., outside the core P0-P8 sequence entirely, no clean number at all. The underlying ticket (T-076: build the SwiftUI NavigationStack shell) is unambiguous and unaffected by this — only the conversational label "Pillar 6" is unreliable.
  - **Resolution (August 20, 2026, later same-day session):** Kell explicitly reconfirmed **Pillar 6 = Menu/Navigation**, overriding the July 11 reassignment to Formation Intelligence. Chose this after being shown the conflict directly (both `PILLAR_6_MENU_BAR_STRATEGY.md` and `PILLAR_6_SEARCH_STRATEGY.md`/T-HYP-P6 also use "Pillar 6," plus the July 11 Formation Intelligence claim) — decided to go with the original May 6 label rather than the July 11 override. New Figma page `P6 — Menu / Navigation` created under this name.
  - **⚠️ Follow-up not yet done:** This re-opens a collision the July 11 session had "resolved" the other way — T-160 through T-164 in this file still carry Epic `Pillar 6 - Formation Intelligence`, and `PILLAR_6_SEARCH_STRATEGY.md`/T-HYP-P6 also claims "Pillar 6." Formation Intelligence and Search now both need a new number/label since Menu has reclaimed Pillar 6. **Not resolved in this session** — flagging so it doesn't silently drift again. Whoever picks this up next should renumber Formation Intelligence (T-160–T-164 Epics) and confirm/rename the Search doc, then treat "Pillar 6 = Menu/Navigation" as final across `docs/MEMORY.md`'s Source-of-Truth Index too.
  - **Dependencies:** None
  - **Estimated effort:** S
  - **Priority:** 🟢 LOW (naming/hygiene, not blocking)
  - **Raised:** August 20, 2026 session
...
**⚠️ Known stale below this line (unchanged since ~May 7, 2026):** this table stops at T-096-era tickets; the real ticket count as of August 20, 2026 is 174 (T-001–T-174, plus S-/V-/BL- series). Treat the highest ticket ID in the file, not this table, as the source of truth for total count. Not recomputing the full breakdown here — would require auditing ~180 tickets across the whole file, out of scope for a session close.
...
**⚠️ Note (August 20, 2026): the P5-P8 walkthrough below is stale — that work completed months ago (see the July 21, 2026 entries near the top of this file: P9/P10/P11/P8/P6-Formation-Intelligence all locked). Left in place for history only. Current objective is below.**

**Start T-076 (Menu/Navigation Shell) — Now That Pillar 1 (Capture) Is Design-Locked End to End**

Pillar 1 happy paths + all 8 error-state scenarios are now fully designed and validated in a working HTML prototype (T-172, this session). Kell's own call this session: pivot to Menu/Navigation next rather than staying heads-down on Capture, since Capture's design work has diminishing returns right now and Menu is what actually makes Capture *reachable* inside a real navigable app.

**Session Opener:**
1. Confirm T-174 first (naming collision) — don't let "Pillar 6" ambiguity carry into this work. Refer to **T-076** directly, not a pillar number.
2. Build T-076 — SwiftUI NavigationStack shell, 4 tabs: **Today | Entries | Create | Growth** (see T-076's own note for the corrected tab names/content owners).
3. T-079 (Wire Create tab → existing `CaptureView`) is the natural follow-on once T-076 exists — this is where T-173 (porting the validated error states) will eventually plug in too.

**Also pending:**
- T-173: Port T-172's validated Capture error states into native `CaptureView.swift`
- T-170/T-171: Onboarding-pacing and loading-copy reviews, now unblocked (Pillar 1 prototype exists to test against)
